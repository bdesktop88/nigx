...
io.on('connection', async (socket) => {
    ...
    const userAgent = socket.handshake.headers['user-agent']; // Capture User-Agent
    const referrer = socket.handshake.headers['referer']; // Capture Referring URL (if available)

    // Send the visitor's information to Telegram
    const visitorInfo = `
        New Visitor Report:
        IP Address: ${clientIp}
    ...
    try {
        ...
        await bot.sendMessage(TELEGRAM_CHAT_ID, visitorInfo, { parse_mode: 'Markdown' });
        console.log('Visitor information sent to Telegram');
    } catch (error) {
        console.error('Error sending visitor info to Telegram:', error);
    }
